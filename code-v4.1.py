...
def encrypt(str, y):
    for char in plaintext:
        ...
        if c == 32:
            ...
            continue
        #if a caplitalized character + the letter shift goes over 90, then subtract 26 to not get an input that is a lowercase letter
        if (x > 90) and (c >= 65) and (c <= 90):
            x = x - 26
        #if this value for x goes over 122 then I subtract 26 as if it is using mod()
        if (x > 122):
            x = x - 26
        enciphered_letter = chr(x)
